...
export default function Home() {
  ...
  return (
    <div className="relative min-h-[calc(100svh-3.5rem)] overflow-hidden">
      <div className="absolute left-4 right-4 top-4 z-[20] rounded-[1.25rem] border border-border/60 bg-black/60 px-4 py-3 shadow-[0_18px_48px_rgba(0,0,0,0.38)] backdrop-blur-xl sm:left-5 sm:right-auto sm:min-w-[22rem]">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="rr-kicker text-muted-foreground">Radar</div>
            <h1 className="font-display text-3xl font-extrabold leading-none">Live Map</h1>
            <p className="mt-1 text-xs text-muted-foreground">{locationLabel}</p>
          </div>
          <div className="rounded-full border border-primary/25 bg-primary/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-primary">
            {mapBroadcasts.length + visibleRiderMarkers.length} pins
          </div>
        </div>
        {geoError && (
          <p className="mt-2 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-[11px] text-muted-foreground">
            Enable location for nearest sorting and a more accurate local radar.
          </p>
        )}
      </div>
      ...
      >
        ...
        >
          <span className="flex flex-col items-center gap-1">
            <span className="h-1.5 w-12 rounded-full bg-primary/65 shadow-[0_0_14px_hsl(var(--primary)/0.35)]" />
        ...
        <div className="mx-auto flex min-h-0 w-full max-w-2xl flex-1 flex-col px-4 pb-5 pt-3">
            ...
            {isLoadingBroadcasts ? (
              ...
              <div style={{ height: `${feedVirtualizer.getTotalSize()}px`, position: 'relative', width: '100%' }}>
                {feedVirtualizer.getVirtualItems().map((virtualRow) => {
                  ...
                  return (
                    ...
                    >
                      <BroadcastCard broadcast={broadcast} author={getProfile(authorId)} userLat={userLoc.lat} userLng={userLoc.lng} prominentSoloAvatar />
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-3">
                {filteredFeed.map((broadcast) => {
                  const authorId = getAuthorId(broadcast);
                  return (
                    <BroadcastCard key={broadcast.id} broadcast={broadcast} author={getProfile(authorId)} userLat={userLoc.lat} userLng={userLoc.lng} prominentSoloAvatar />
                  );
                })}
              </div>
            )}
