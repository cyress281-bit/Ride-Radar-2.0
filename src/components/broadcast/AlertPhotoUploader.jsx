...
/**
 * AlertPhotoUploader - Handles photo upload for alert broadcasts.
 *
 * Accessibility:
 * - File inputs have proper aria-labels
 * - Upload progress announced via aria-live region
 * - Error messages linked to inputs via aria-describedby
 * - Keyboard users can tab to all interactive elements
 * - Remove buttons have descriptive aria-labels
 * - Drag targets have visual focus indicators
 *
 * Edge cases:
 * - Handles rapid sequential uploads without race conditions
 * - Invalid file types show clear error messages
 * - Large files are caught by validation before upload
 * - Gracefully handles when prepareLocalImage throws
...
export default function AlertPhotoUploader({ images = [], onChange, disabled = false }) {
  ...
  const slots = photos.length < MAX_PHOTOS ? [...photos, null] : photos;
  const canAddMore = photos.length < MAX_PHOTOS && !disabled;

  const uploadAt = useCallback(async (file, index) => {
  ...
  return (
    <fieldset className="border-0 p-0 m-0" disabled={disabled}>
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Attach up to {MAX_PHOTOS} safety photos (JPG, PNG, or WebP). Photos are optional.
      </p>
    </fieldset>
  );
}
