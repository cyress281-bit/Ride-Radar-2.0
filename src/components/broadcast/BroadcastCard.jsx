...
const BroadcastCard = memo(function BroadcastCard({ broadcast, author, userLat, userLng, prominentSoloAvatar = false }) {
  ...
  const distance =
    (broadcast.type === 'solo_ride' || broadcast.type === 'iso') && userLat != null && broadcast.frozen_lat != null
      ? formatDistance(haversineMiles(userLat, userLng, broadcast.frozen_lat, broadcast.frozen_lng))
      : null;

  const isoSubLabel =
    broadcast.iso_subtype === 'mechanic' ? 'Mechanic' : broadcast.iso_subtype === 'bike_crew' ? 'Bike Crew' : null;

  const content = (
    <div className={cn('relative rounded-[1.15rem] overflow-hidden transition-all duration-300 rr-scanline active:scale-[0.992]', styles.card, 'hover:' + styles.leftGlow)}>
      {/* Alert top bar pulse */}
      ...
      <div className="relative p-4 md:p-5">
        ...
        {broadcast.type === 'alert' && <AlertPhotoGrid images={broadcast.alert_photos} />}
            ...
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground flex-wrap mt-2.5 font-mono tracking-tight">
              ...
              {(broadcast.type === 'event' || isAlert) && broadcast.exact_location_text && (
                <span className="inline-flex items-center gap-1">
                  <MapPin className="w-3 h-3 opacity-60" />{broadcast.exact_location_text}
                </span>
              )}
              ...
              <span className="inline-flex items-center gap-1 ml-auto tabular-nums">
                <Clock className="w-3 h-3 opacity-60" />{timeUntilExpiry(broadcast.expires_at)}
              </span>
            </div>
          </div>
        </div>
