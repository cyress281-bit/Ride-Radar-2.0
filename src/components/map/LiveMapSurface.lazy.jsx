...
// This version lazy-loads leaflet & react-leaflet at runtime to avoid loading
// the heavy library until the map component mounts.
...
export default function LiveMapSurfaceLazy({
  broadcasts = [],
  presenceMarkers = [],
  getProfile,
  userLat,
  ...
  const [RL, setRL] = useState(null); // react-leaflet module
  const mountedRef = useRef(true);

  // caches for generated icons
  const markerIconCacheRef = useRef(new Map());
  const riderMarkerIconCacheRef = useRef(new Map());

  useEffect(() => {
    mountedRef.current = true;
    let cancelled = false;

    // Dynamically import leaflet and react-leaflet when the map surface mounts.
    Promise.all([import('leaflet'), import('react-leaflet')])
      ...
      .catch((err) => {
        console.error('[LiveMapSurface] failed to load leaflet modules', err);
        setMapError(true);
      });
  ...
  const getRiderMarkerIcon = useCallback(
    (presence) => {
      if (!L) return null;
      const cache = riderMarkerIconCacheRef.current;
  ...
  if (!leafletLoaded) return <LoadingState variant={variant} />;
  if (items.length === 0) return <EmptyState variant={variant} />;

  const { MapContainer, Marker, Popup, TileLayer, useMap } = RL;
  ...
  function FitMapToItems({ items, userLat, userLng, variant }) {
    ...
    useEffect(() => {
      ...
      if (items.length === 0) {
        map.setView(center, variant === 'full' ? 4 : 3);
        return;
      }
      ...
      map.fitBounds(items.map((item) => [item.lat, item.lng]), {
  ...
  function EmptyState({ variant }) {
    return (
      ...
      >
        ...
        <p className="mt-1 max-w-sm text-sm text-muted-foreground">Active broadcasts with recognizable locations will appear here as soon as they hit the network.</p>
      </div>
    );
  }
