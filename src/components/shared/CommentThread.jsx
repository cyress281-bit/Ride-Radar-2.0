...
const CommentThread = memo(function CommentThread({
  ...
  const composer = (
    <div
      className={
        dockComposer
          ? 'fixed bottom-0 left-0 right-0 z-20 flex items-center gap-2 px-4 py-3 border-t border-white/[0.06] bg-surface/95 backdrop-blur-xl'
          : stickyComposer
      ...
      style={
        dockComposer
          ? {
              transform: 'translateY(calc(-1 * var(--rr-keyboard-height, 0px) + env(safe-area-inset-bottom, 0px)))',
              paddingBottom: 'max(0.75rem, calc(env(safe-area-inset-bottom, 0px) - var(--rr-keyboard-height, 0px)))',
