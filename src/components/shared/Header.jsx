...
/**
 * Header — Electric neon scroll-aware header with glassmorphism.
 *
 * Design:
 * - Transparent at top, gains bg-background/80 backdrop-blur on scroll
 * - Left: Back button (when not on home) OR Logo (on home)
 * - Center: Page title in neon green
 * - Right: Notifications bell + Avatar
 * - Neon green accents on active/hover states
 */
export const Header = memo(function Header({ isOverlay = false }) {
      ...
      >
          ...
          {isRadar ? (
            ...
            pageTitle && (
              <Text
                variant="h3"
                color="default"
                className="text-base font-bold text-primary rr-neon-green"
              >
                {pageTitle}
              </Text>
            )
        ...
        <HStack align="center" gap={1} role="group" aria-label="Header actions">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span
                className="absolute top-2 right-2 h-2 w-2 rounded-full bg-brand-emergency ring-2 ring-background shadow-[0_0_6px_hsl(var(--destructive)/0.6)]"
                aria-hidden="true"
              />
            )}
          </NavLink>
