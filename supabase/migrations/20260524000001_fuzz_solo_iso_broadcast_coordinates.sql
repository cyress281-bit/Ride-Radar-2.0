...
BEGIN
  ...
  SELECT
    ...
    -- Multiplying by 0.015° gives ±0.015° offset (~1 mile at US latitudes).
    ...
    CASE
      ...
      THEN c.frozen_lat + (hashtext(c.id::text || ':lat') / 2147483647.0) * 0.015
      ELSE c.frozen_lat
    END AS frozen_lat,
    CASE
      WHEN c.type IN ('solo_ride', 'iso')
        AND c.author_id IS DISTINCT FROM auth.uid()
      THEN c.frozen_lng + (hashtext(c.id::text || ':lng') / 2147483647.0) * 0.015
      ELSE c.frozen_lng
    END AS frozen_lng,
    c.location_name,
    c.event_date,
