...
### Approved Task for Kimi — Navigation Speed (F1 broadcast+rider, F1c messages, F2, F3, F5)
**Status: DONE — All phases executed, committed, and pushed. Build (lint + typecheck + build) passed for all phases. Awaiting owner on-device verification (chat messages prefetch must be tested on iPhone PWA).**
