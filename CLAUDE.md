...
**Status:** ✅ **DONE — VERIFIED ON DEVICE BY OWNER (2026-05-31).** Final shipped design = the **"Alive Logo"** loader (the B→C cinematic was scrapped on device for overlap/clutter). Single unified `PageLoader` everywhere: centered `logo.png` that gently breathes (scale-only, no glow box), scaled to ~`104vw` so the green art fills the width with a small margin (black padding overflows invisibly), one-time bloom-in intro on first launch, clean `#040406` pre-paint, reduced-motion static, full a11y. Owner: "looks great I love it." (The B→C exploration + Codex review below is preserved for the record.) Tunable knob if ever revisited: the `w-[104vw]` value in `src/components/shared/PageLoader.jsx`.
