...
ESLint runs on `src/components/**`, `src/features/**`, `src/hooks/**`, `src/providers/**`, `src/utils/**`, `src/App.jsx`, and `src/main.jsx` (excludes `src/lib/` and `src/components/ui/`). Rules: no unused imports (auto-removed via `eslint-plugin-unused-imports`), React hooks rules, no prop-types required (uses JSDoc for types).
