...
**Status:** ✅ **DONE — VERIFIED BY OWNER (2026-05-31).** Dual-pass inspection (Kimi swarm + Claude Code) → reconciled → shipped. **Shipped & verified:** MapLibre 918 KB chunk-leak fix (`439bbda`); F1 data-prefetch on intent for broadcast detail + rider profile (`f863e52`); F2 BottomNav tab preload (`8fa023d`); F3 `gcTime` 5min→30min (`d3bf9fa`); F5 dead `vendor-leaflet` chunk removed (`4c4dc47`); **F1c messages prefetch (`29d0bc4`)** — helper aligned to `useMessages` exact key + `getMessages()` (signed-image hydration), wired to `ConversationItem`; **A (`71208f6`)** — top-level `Suspense fallback={null}` → `<RouteTransitionFallback />` so public routes (`/login`, `/landing`, legal) no longer flash blank on cold chunk load; **B (`71208f6`)** — `preloadCoreRoutes` secondary preload delay reduced 1000ms → 100ms so core chunks warm sooner after boot. All build-green; no Protected Behaviors touched. **Kimi#5 (BroadcastDetail 2-query waterfall) is HELD — owner directed Kimi to stop; needs Claude/Codex review before any work.** Kimi's full findings: `audit-listings/navigation-speed-findings.md`. The dual-inspector cross-check paid off: Kimi caught the MapLibre leak Claude missed; Claude caught the prefetch key-misalignment Kimi missed.
...
The HIGH-impact safe gap actioned below: **data-prefetch helpers in `src/lib/query-client.js` are defined but never called AND are misaligned with the real page hooks.** Approved Task = wire them (aligned) + small preload/cache/cleanup wins. **Messages prefetch was HELD then approved and shipped (see F1c below).**

### Approved Task for Kimi — Navigation Speed (F1 broadcast+rider, F1c messages, F2, F3, F5, A, B)
**Status: DONE — All approved phases executed, committed, and pushed. Build (lint + typecheck + build) passed for all phases. Awaiting owner on-device verification (chat messages prefetch must be tested on iPhone PWA).**

**Held for Claude/Codex review (owner-directed stop):**
- Kimi#5: BroadcastDetail 2-query waterfall — changes data-fetching contract (sequential → parallel), could break loading states. Not approved for Kimi execution.
- Prior audit holds remain: C-009 ViewportProvider memoization, H-027 44px touch targets, C-011 raw `<img>` → `OptimizedImage`.
...
1. **Fix the two helpers in `src/lib/query-client.js`** so their queryKey + queryFn are identical to the consuming page (otherwise the prefetch never hits the page's query):
   ...
   - `prefetchConversationMessages(qc, conversationId, userId)` was held then approved separately — see F1c below.
...
#### F1c — Messages prefetch (APPROVED → DONE)
**Plan:** Codex designed alignment; Claude Code reviewed and green-lit with two refinements: (1) helper must match `useMessages` exact query key + `getMessages()` queryFn, (2) `seenIdsRef` sync is defensive hygiene (not strictly required because the realtime handler already has a `.some()` duplicate guard). Owner directed Kimi to execute.

**Shipped in `29d0bc4`:**
1. `src/lib/query-client.js`: `prefetchConversationMessages(qc, conversationId, userId)` — key changed from `['messages', conversationId]` → `['messages', conversationId, userId]`; queryFn changed from raw `supabase.select('*')` to `getMessages(conversationId)` (preserves signed-image URL hydration via `hydrateMessageImages`). Removed now-unused `supabase` import.
2. `src/features/chat/components/ConversationItem.jsx`: wired data prefetch as 2nd arg to existing `withRoutePreload(preloadConversation, () => prefetchConversationMessages(qc, conversation.id, user.id))`. Only fires when `user?.id && conversation?.id`.
3. `src/features/chat/hooks/use-messages.js`: added defensive `useEffect` that seeds `seenIdsRef.current` from `query.data` on every data change. Covers the case where prefetched cache is returned without re-running `queryFn`.

**NOT shipped (Phase 2, still held):** seeding `['conversation', id, userId]` from the list data. Deferred until shape parity is verified — the `conversation` object from `getConversations` may not contain every field `ConversationPage` reads from its separate `useQuery(['conversation', id, user?.id])`.

**Verify on device (iPhone PWA):** open `/messages`, tap most recent chat → messages should render faster (data already warmed); no duplicate messages; keyboard/input behave normally; sending still works.
...
|---|---|---|
| 2026-05-31 | Kimi | **Shipped F1c messages prefetch (chat) — DONE, awaiting device verification.** Commit `29d0bc4`: aligned `prefetchConversationMessages` to match `useMessages` exactly — key `['messages', conversationId, userId]` + `getMessages(conversationId)` (preserves signed-image hydration); wired via `withRoutePreload` 2nd arg in `ConversationItem.jsx`; added defensive `seenIdsRef` sync effect in `use-messages.js` for prefetched-cache mount case. Phase 2 (conversation-detail seeding from list data) remains HELD pending shape-parity check. Build green (lint + typecheck + build, EPERM workaround used). No Protected Behaviors touched. **Owner on-device check required:** tap a chat from `/messages` → faster message render, no duplicates, keyboard normal, sending works. |
| 2026-05-31 | Kimi | **Shipped Navigation Speed A + B — DONE.** Commit `71208f6`: (A) top-level `Suspense fallback={null}` → `<RouteTransitionFallback />` — public routes (`/login`, `/landing`, legal pages) no longer flash blank white while their lazy chunk loads on cold start; exported `RouteTransitionFallback` from `AppLayout.jsx` to make it available at the `App.jsx` level. (B) `preloadCoreRoutes` secondary delay reduced 1000ms → 100ms — `preloadHome()` still fires immediately on idle, but `preloadMessages`/`preloadBroadcast`/`preloadProfile` now start ~100ms later instead of ~1s later, helping fast-tapping users. Both are behavior-neutral perf polish; no Protected Behaviors touched. Build green. |
