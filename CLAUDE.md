...
**Status:** ✅ **DONE — VERIFIED BY OWNER (2026-05-31).** Dual-pass inspection (Kimi swarm + Claude Code) → reconciled → shipped. **Shipped & verified:** MapLibre 918 KB chunk-leak fix (`439bbda`); F1 data-prefetch on intent for broadcast detail + rider profile (`f863e52`); F2 BottomNav tab preload (`8fa023d`); F3 `gcTime` 5min→30min (`d3bf9fa`); F5 dead `vendor-leaflet` chunk removed (`4c4dc47`); **F1c messages prefetch (`29d0bc4`)** — helper aligned to `useMessages` exact key `['messages', conversationId, userId]` + `getMessages()` (preserves signed-image hydration), wired to `ConversationItem` via `withRoutePreload` 2nd arg, `seenIdsRef` defensive sync added. All build-green; no Protected Behaviors touched. **Remaining (intentional):** BroadcastDetail 2-query waterfall (Kimi#5), public-route blank-on-load (Kimi#6). Kimi's full findings: `audit-listings/navigation-speed-findings.md`. The dual-inspector cross-check paid off: Kimi caught the MapLibre leak Claude missed; Claude caught the prefetch key-misalignment Kimi missed.
